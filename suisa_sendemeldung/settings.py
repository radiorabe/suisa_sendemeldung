...
class EmailSettings:
    ...
    sender: str = ts.option(help="the sender of the email")
    to: str = ts.option(help="the recipients of the email")
    server: str = ts.option(help="the smtp server to send the mail with")
    username: str = ts.option(
        help="the username to logon to the smtp server (default: email_from)"
    )
    password: str = ts.secret(help="the password for the smtp server")
